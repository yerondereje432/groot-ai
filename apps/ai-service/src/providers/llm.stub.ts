...
export class StubLLMProvider implements LLMProvider {
  ...
  private compose(req: LLMCompletionRequest): string {
    const { prompt } = req;
  ...
  private extractSentences(text: string, max: number): string[] {
    const cleaned = text
      .replace(/\s+/g, ' ')
      .split(/(?<=[.!?])\s+/)
      .map(s => s.trim())
  ...
  private composeQuestions(sentences: string[], locale: 'am' | 'en'): string {
