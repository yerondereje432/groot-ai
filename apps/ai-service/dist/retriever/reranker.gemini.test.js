...
function makeHit(content, topicId, score) {
    return {
        chunk: {
...
function mockGeminiResponse(scores) {
    return {
        ok: true,
        statusText: 'OK',
        json: async () => ({
...
describe('GeminiReRanker', () => {
    ...
    it('returns at most topK results', async () => {
        globalThis.fetch = vi.fn().mockResolvedValue(mockGeminiResponse([{ index: 0, score: 0.9 }, { index: 1, score: 0.1 }, { index: 2, score: 0.8 }]));
    ...
    it('ranks by Gemini relevance score when the call succeeds', async () => {
        globalThis.fetch = vi.fn().mockResolvedValue(mockGeminiResponse([{ index: 0, score: 0.1 }, { index: 1, score: 0.95 }]));
        const r = new GeminiReRanker({ apiKey: 'test-key' });
    ...
    it('applies the topic metadata boost on top of the LLM score', async () => {
        globalThis.fetch = vi.fn().mockResolvedValue(mockGeminiResponse([{ index: 0, score: 0.5 }, { index: 1, score: 0.5 }]));
        const r = new GeminiReRanker({ apiKey: 'test-key' });
...
//# sourceMappingURL=reranker.gemini.test.js.map