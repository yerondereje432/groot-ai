...
function buildOrchestrator(minConfidence = 0.35) {
    ...
    const retriever = new Retriever({ embedder, store, reranker: new StubReRanker(), cache }, { ...DEFAULT_RETRIEVER_CONFIG, minConfidence });
    return new Orchestrator({ llm: new StubLLMProvider(), retriever, generateSessionId: () => nanoid() }, { ...DEFAULT_RETRIEVER_CONFIG, minConfidence });
}
