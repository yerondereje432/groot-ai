...
export class GeminiOcrParser {
  ...
  async parse(filename: string, content: Buffer): Promise<ParsedDocument> {
    ...
    const prompt =
      `This PDF is a scanned page from an Ethiopian high-school textbook. It may be in ` +
    ...
    const rawSections = JSON.parse(jsonMatch[0]) as OcrSection[];
 ...
 * just "is it empty".
 */
export function looksLikeScannedDocument(extractedText: string, pageCount: number): boolean {
